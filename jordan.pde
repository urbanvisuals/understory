...
void expandingSquares_draw(){
  ...
  background(0);
  rectMode(CENTER);
  translate(normX*2400, -normY*600+600);   //translate to x and y input
  //rotate(PI/3);     //set to fader or set to rotate automatically
  rotate((2*PI*fader3)/255);     //set to fader or set to rotate automatically
  rotate((frameCount * fader4)/20000.0);
...
void expandingEllipses_setup(){
  noStroke();
  fill(red1, green1, blue1); 
}

void expandingEllipses_draw(){
  speed = map(fader1, 0, 255, 0, 10);  // set to fader
  gap = map(fader2, 0, 255, 3, 500);    // set to fader
  pushMatrix();
  int numRects = 4400/int(gap);
  ...
  for (int i = numRects; i > -2 ; i--) {
    ...
    println(i);
    if (i*gap+size > 0){
      ellipse(0, 0, i*gap+size, i*gap+size);
    }
  }
  size = size + speed;
  if (size > gap*2)size=0;
...
Scene expandingEllipses = new Scene("expandingEllipses", "Expanding Ellipses") {
  void setup() { expandingEllipses_setup(); }
  void draw() { expandingEllipses_draw(); }
};
